...
GROUND_RECT = Rect((0, 550), (WIDTH, 30))
...
class Enemy:
    def __init__(self, idle_images, walk_images, position, velocity=1.5):
        self.idle_images = idle_images
        self.walk_images = walk_images
        self.actor = Actor(self.idle_images[0], pos=position)
...
portal = Actor("portal", pos=(last_platform.centerx + 95, last_platform.top - 60))
...
def draw_menu(screen):
    ...
    screen.draw.filled_rect(button_start, (50, 200, 100))
    screen.draw.filled_rect(button_music, (50, 150, 100))
    screen.draw.filled_rect(button_quit, (200, 50, 50))

    screen.draw.text("Start Game", center=button_start.center, fontsize=36, color="white")
    screen.draw.text(f"Music: {'ON' if music_on else 'OFF'}", center=button_music.center, fontsize=36, color="white")
    screen.draw.text("Exit", center=button_quit.center, fontsize=36, color="white")
...
def draw():
    ...
    screen.draw.text("Get to the Portal and avoid the slimes!", center=(WIDTH//2, 30), fontsize=24, color="red")
